...
class _HazardAwareHeuristic:
    ...
    def __call__(self, state: PlanningState, goal: GoalSpec) -> float:
        ...
        penalty = (
            (self.w_hazard * (1.0 + self.slip_weight_gain * slip_p)) if hazard else 0.0
        )
        return max(-1.0, min(0.999, self.w_progress * progress - penalty))


def _measure_mean_reward(
    heuristic: _HazardAwareHeuristic, model: Any, *, seeds: range
) -> float:
    ...
    for seed in seeds:
        strategy = MCTSStrategy(
            exploration=MCTSExploration(iterations=128, rollout_depth=6, seed=seed),
...
class TestSlipAwareHeuristicLibraryPattern:
    def test_slip_aware_reaches_hazard_state_during_rollout(self) -> None:
        ...
        heuristic = _HazardAwareHeuristic(
            transition_model=_CorridorSlipModel(slip_p=0.3)
        )
        hazard_state = PlanningState.from_dict(
            {"pos": 4, "hazard": True, "alive": True}
        )
        plain_state = PlanningState.from_dict(
            {"pos": 4, "hazard": False, "alive": True}
        )
        assert heuristic(hazard_state, GOAL) < heuristic(plain_state, GOAL)

    def test_slip_aware_penalty_scales_with_slip_p(self) -> None:
        heuristic_low = _HazardAwareHeuristic(
            transition_model=_CorridorSlipModel(slip_p=0.1)
        )
        heuristic_high = _HazardAwareHeuristic(
            transition_model=_CorridorSlipModel(slip_p=0.5)
        )
        hazard_state = PlanningState.from_dict(
            {"pos": 4, "hazard": True, "alive": True}
        )
    ...
    def test_deterministic_model_preserves_slip_blind_behaviour(self) -> None:
        ...
        un-inflated."""
        heuristic_det = _HazardAwareHeuristic(
